...
def get_personal_info(blocks, block_dict):
    Insured_dict = {}
    Agent_dict = {}
    Name = []
    Agent_Name = []
...
def get_company_info(blocks, block_dict):
    Company_dict = {}
...
def get_driver_info(blocks, block_dict):
    driver_info_dict = {}
    ...
    if 'Comprehensive Deductible' in block_dict.keys():
        for x in range(1, v + 1): Comp.append(
    ...
    if 'Collision Deductible' in block_dict.keys():
        for x in range(1, v + 1): Coll.append(
    ...
    for f in file:
        if '.pdf' in f:
            ...
            list = ['Name', 'Address', 'Phone Number', 'Phone', 'Phone Number ( ) -', 'Work', 'Work Number', 'Company',
                    ...
                    'Collision Deductible', 'Liability BI', 'Liability PD', 'Uninsured BI', 'Unins PD/Coll Ded','Unins PD/Coll Ded Waiver']
            block_dict = check_blocks(blocks, list)
            print("----PERSONAL INFO-----")
            Insured_info, Agent_info = get_personal_info(blocks, block_dict)
            print(Insured_info)
            print(Agent_info)
            print("----Company INFO-----")
            Company_info = get_company_info(blocks, block_dict)
            print(Company_info)
            Driver_info = get_driver_info(blocks, block_dict)
            print(Driver_info)
